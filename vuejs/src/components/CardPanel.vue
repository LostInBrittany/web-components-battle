	...
	<div>
		<md-card class="card-panel">
			<md-card-header>
				<md-card-header-text>
					<div class="md-title">
						<router-link :to="{ name: 'detail', params: { id: person.id }}" class="card-title username">
			...
			<md-card-content>
				<md-layout>
					...
					<md-layout md-flex-offset="45">
						...
						<router-link :to="{ name: 'map', params: { id: person.id }}">
							<md-icon class="md-accent">map</md-icon>
						</router-link>
						<a @click="onDelete">
				...
				<div v-if="showDetail" class="skills">
					<h3>Skills</h3>
					<a class="btn-large" v-for="skill in person.skills">{{ skill }}</a>
				</div>
				<div v-if="showDetail" class="links row">
					<a v-for="link, service in person.links" :href="link">
